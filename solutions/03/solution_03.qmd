...
```{r}
m_peng_1 <- lm(flipper_length_mm ~ body_mass_g + species, data = penguins)
```
...
**pente globale:** 8.515e-03
**ordonnée Adelie:** 1.585e+02 (reference)
**ordonnée Chinstrap:** 1.585e+02 + 5.492e+00 = 163.992
**ordonnée Gentoo:** 1.585e+02 + 1.533e+01 = 173.83
...
ggplot(
  ...
  labs(x = "Body Mass (g)", y = "Flipper Length (mm)") +
  geom_ribbon(
    data = pred_m_peng_1, #<2>
    aes(ymin = CI_low, ymax = CI_high, y = NULL, col = NULL, fill = species), #<3>
    alpha = 0.2,
  ) +
  geom_line(data = pred_m_peng_1, aes(x = body_mass_g, y = Predicted)) + #<4>
  theme_light()
```
...
ggplot(
  ...
  labs(x = "Body Mass (g)", y = "Flipper Length (mm)") +
  geom_ribbon(
    data = pred_m_peng_2, #<2>
    aes(ymin = CI_low, ymax = CI_high, y = NULL, col = NULL, fill = species), #<3>
    alpha = 0.2,
  ) +
  geom_line(data = pred_m_peng_2, aes(x = body_mass_g, y = Predicted)) + #<4>
  theme_light()
