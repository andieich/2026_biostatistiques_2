---
title: Distributions statistiques alternatives
subtitle: Cours 6 
---
...
## Forme des distributions - **Normal** {.smaller}

```{r}

base_theme <- theme_light(base_size = 16) +
  theme(panel.grid = element_blank(), legend.position = "top")

normal_params <- tribble(
  ~mu , ~sigma ,
    0 , 0.7    ,
    0 , 1.2    ,
    2 , 1.0
) |>
  mutate(label = paste0("μ = ", mu, ", σ = ", sigma))

gamma_params <- tribble(
  ~alpha , ~beta ,
  1.0    , 1.0   ,
  2.0    , 1.0   ,
  5.0    , 1.0   ,
...
poisson_params <- tibble(lambda = c(1, 4, 10)) |>
  mutate(label = paste0("λ = ", lambda))

df_normal <- crossing(x = seq(-4.5, 6.5, length.out = 600), normal_params) |>
  mutate(y = dnorm(x, mean = mu, sd = sigma))

df_gamma <- crossing(x = seq(0, 20, length.out = 600), gamma_params) |>
  mutate(y = dgamma(x, shape = alpha, rate = beta))

df_poisson <- crossing(x = 0:20, poisson_params) |>
  mutate(y = dpois(x, lambda = lambda))

p_normal <- ggplot(df_normal, aes(x, y, color = label)) +
  geom_line(linewidth = 1.2) +
  labs(x = "x", y = "Density", color = NULL) +
  base_theme

p_gamma <- ggplot(df_gamma, aes(x, y, color = label)) +
  geom_line(linewidth = 1.2) +
  labs(x = "x", y = "Density", color = NULL) +
  base_theme

p_poisson <- ggplot(df_poisson, aes(x, y, color = label, group = label)) +
  geom_line(color = "grey55", linewidth = 1.1) +
  geom_point(size = 3.2, stroke = 1, shape = 21, fill = "white") +
  labs(x = "x", y = "P(X = x)", color = NULL) +
  base_theme
...
## Forme des distributions - **Gamma** {.smaller}
...
## Forme des distributions - **Poisson** {.smaller}
