...
-   Gardez à l'esprit que ce n'est pas ici les valeurs attendues ($\mathrm{Mortalité}_i \sim \mathrm{Normale(\mu}_i,~\mathrm{\sigma)}$) incluant $\mathrm{\sigma}$ qui sont affichées, mais la certitude du modèle à estimer la partie $\mathrm{\mu}_i$
...
Charger les données contenant des informations sur le genre aussi
