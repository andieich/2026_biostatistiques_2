---
title: "TP 04-2 : Hypothèses des modèles"
format: html
editor: visual
---
...
```{r}
coral_transects <- read.csv(here("TP/TP_04/data/coral_cover_transects.csv"))
```
...
#### Décaler ceci vers les indices

**Remarque :** Les résultats sont différents ! Auquel feriez-vous plus confiance ? Le deuxième modèle, car il respecte l'hypothèse d'indépendance—chaque point de données représente un site indépendant, pas trois pseudo-réplicats.

**Leçon clé :** Pensez toujours à votre conception d'étude. Si les observations sont regroupées ou groupées, agrégez au sein des groupes AVANT d'ajuster le modèle.
...
*Comment vérifier :* Les points devraient tomber le long de la ligne diagonale.
