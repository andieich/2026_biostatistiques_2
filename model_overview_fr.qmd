...
models_data <- tibble(
  ...
  `Génération des données` = c(
    "$y \\sim \\mathrm{Normal}(\\mu, \\sigma)$<br>$\\mu = a + b x$",
    "$y \\sim \\mathrm{Normal}(\\mu, \\sigma)$<br>$\\mu = \\mu_A$",
    "$y \\sim \\mathrm{Normal}(\\mu, \\sigma)$<br>$\\mu = \\mu_A + \\mu_B$",
    "$y \\sim \\mathrm{Normal}(\\mu, \\sigma)$<br>$\\mu = \\mu_A + \\mu_B + \\mu_{AB}$",
    "$y \\sim \\mathrm{Normal}(\\mu, \\sigma)$<br>$\\mu = a + b x + \\mu_A$",
    "$y \\sim \\mathrm{Normal}(\\mu, \\sigma)$<br>$\\mu = a_A + b_A x$"
  ),
  Variables = c(
    "• 1 numérique (x)",
    "• 1 catégorique (A)",
  ...
  Description = c(
    ...
    "Évaluer l'impact de 2 facteurs ; l'effet de A dépend de B (ou l'inverse)",
    "Comparer les ordonnées à l'origine entre groupes sur la même droite de régression (droites parallèles, mêmes pentes)",
    "Comparer les pentes de régression et ordonnées à l'origine selon les groupes"
  ),
  `Exemple Manchot` = c(
...
gt_table <- gt(models_data) %>%
  cols_width(
    Name ~ pct(6),
    Formula ~ pct(11),
    `Génération des données` ~ pct(11),
    Variables ~ pct(16),
    Description ~ pct(14),
    `Exemple Manchot` ~ pct(10),
    `Exemple générique` ~ pct(10),
    Plot ~ pct(17)
  ) %>%
  tab_options(
    table.width = pct(100),
  ...
  tab_style(
    style = cell_text(align = "left", v_align = "top", size = "9.6px"),
    locations = cells_body(columns = c(Formula, Variables))
  ) %>%
  tab_style(
    style = cell_text(align = "left", v_align = "top", size = "9.6px"),
    locations = cells_body(columns = c(`Génération des données`))
  ) %>%
  tab_style(
  ...
  tab_source_note(
    source_note = md(
      "**ANOVA** = **a**nalysis of **va**riance ; **ANCOVA** = **an**alysis of **cova**riance"
    )
  ) %>%
